<?php
include '../config.php';
set_data("TRANSCRIPTS,id=$_SESSION[userid]");
...
mysqli_query($con,"INSERT into $table (tsfile,player_id,title) VALUES ('$target_file',$_SESSION[userid],'$_POST[tsname]')");

set_data("TRANSCRIPTS,player_id=$_SESSION[userid]");
$tscripts=collection();
foreach ($tscripts as $tsout)
{
    echo '<p><a class="transcripts" href="'.$tsout[tsfile].'" target="_blank">'.$tsout[title].'</a></p>';
}
}
?>